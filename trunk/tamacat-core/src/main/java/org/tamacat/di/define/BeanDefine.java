...
public class BeanDefine implements Cloneable {
    ...
    public void setType(String className, ClassLoader loader) {
        this.type = (loader != null) ?
            ClassUtils.forName(className, loader): ClassUtils.forName(className);
    }
    ...
    /**
     * Set the alias of id.
     * (Two or more values can also be set by comma separated values.)
     */
    public void setAliases(String aliases) {
        if (aliases == null || aliases.trim().length() == 0) return;
