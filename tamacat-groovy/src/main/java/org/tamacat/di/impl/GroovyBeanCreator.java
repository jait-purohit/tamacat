 ...
 */
public class GroovyBeanCreator {
    ...
    private final GroovyLoader loader;

    /**
     * Constructor for DIContainer
     * @param defines
     */
    GroovyBeanCreator(BeanDefineMap defines) {
        this.defines = defines;
        this.loader = GroovyFactory.getGroovyLoader();
        initDefines();
    }
     ...
     * @param defines
     */
    GroovyBeanCreator(BeanDefineMap defines, GroovyLoader loader) {
        this.defines = defines;
        this.loader = loader;
        initDefines();
    }
        ...
        for (int i=0; i<argsDef.size(); i++) {
            if (argsDef.get(i).isRef()) {
                ...
                if (argsDef.get(i).getType() == null) {
                    ...
                    argsTypes[i] = loader.loadClass(argsDef.get(i).getType());
                }
            }
        }
